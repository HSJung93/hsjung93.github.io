...
코어를 늘릴수록 느려진다는 것은 워커들이 어딘가에서 직렬화되고 있다는 신호다. 병렬 작업하는 워커들이 하나의 잠금으로 수렴하는 지점을 찾아야 했다.

causalml의 트리 학습은 속도를 위해 Cython으로 작성되어 있다. 그 소스 5개의 `.pyx`(와 대응하는 `.pxd` 헤더)에 다음 디렉티브가 하드코딩되어 있었다.
...
모델이 옳은 답을 주는지만큼이나 그 답을 제때 주는지도 중요하다고 생각한다. 느린 도구는 충분히 쓰이지 못하고, 그래서 충분히 검증되지도 못하니까 말이다. 
